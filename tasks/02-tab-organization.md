...
- [ ] Таб автоматически получает имя из первой непустой строки содержимого.
- [ ] Автоимя не перетирает ручное имя таба.
- [ ] Уже существующие `Untitled N` табы получают автоимена при следующей загрузке/изменении, если у них есть содержимое.
- [ ] В UI есть fuzzy-поиск по табам, который ищет по title и content preview.
- [ ] Из fuzzy-поиска можно перейти к выбранному табу клавиатурой.
- [ ] В fuzzy-поиске видны dirty-индикатор, title и короткий preview.
- [ ] Есть действие cleanup пустых табов, которое закрывает только безопасно пустые табы.
- [ ] Cleanup не удаляет активный единственный таб; после cleanup в приложении всегда остаётся хотя бы один таб.
- [ ] Cleanup показывает toast с количеством закрытых табов.
- [ ] Горячие клавиши отображаются в `ShortcutsModal`.
